...
1. Verify `specs/derived-spec/` exists and contains the expected files.
2. Read each derived spec file.
3. Generate `src/` with module files based on `specs/derived-spec/architecture.md`.
4. Generate `tests/` with test files based on `specs/derived-spec/tests.md`.
5. Generate `package.json` based on `specs/derived-spec/implementation.md`.
